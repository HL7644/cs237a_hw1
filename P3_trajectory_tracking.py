...
class TrajectoryTracker:
    ...
    def compute_control(self, x: float, y: float, th: float, t: float) -> T.Tuple[float, float]:
        ...
        ########## Code starts here ##########
        #step1) compute virtual control inputs w.r.t error vector
        if self.V_prev<V_PREV_THRES:
            #if uninitialized: set to desired trajectory's velocity
            self.V_prev=np.sqrt(xd_d**2+yd_d**2)
        x_dot=self.V_prev*np.cos(th)
        y_dot=self.V_prev*np.sin(th)
        w1=xdd_d-self.kpx*(x-x_d)-self.kdx*(x_dot-xd_d)
        w2=ydd_d-self.kpy*(y-y_d)-self.kdy*(y_dot-yd_d)
        vel=np.sqrt(x_dot**2+y_dot**2)
        J_inv=torch.FloatTensor([[np.cos(th), np.sin(th)],[-np.sin(th)/vel, np.cos(th)/vel]]).to(device)
        virt_output=torch.FloatTensor([[w1],[w2]]).to(device)
        output=J_inv.matmul(virt_output).squeeze(dim=-1)
        acc=output[0].item()
        V=self.V_prev+acc*dt
        om=output[1].item()
        ########## Code ends here ##########
